...
STATIC_INLINE void pepper_graphics3d_flush_screen(struct vidbuf_description *gfxinfo,
    ...
    //pp::CompletionCallback cc(SwapDone, _this, 0);
    ppb_g3d_interface->SwapBuffers(graphics_context, PP_BlockUntilComplete());

    /* Throttle to 50 frames per second. */
    /* TODO(cstefansen): Fix frame throttling when PNaCl nanosleep/usleep
     * functionality is confirmed to work. Currently, the frame rate is
     * throttled by finish_sound_buffer in sd-pepper/sound.c, but this is
     * less than ideal, as it only provides throttling when sound is
     * playing. */
//    static const long int kMicrosecondsPerFrame = 20000;
//    gettimeofday(&tv_now, NULL);
//    long int time_spent_frame = (tv_now.tv_sec * 1000000 + tv_now.tv_usec) -
//            (tv_prev.tv_sec * 1000000 + tv_prev.tv_usec);
//    long int microSecondsAhead = kMicrosecondsPerFrame - time_spent_frame;
//    if (microSecondsAhead > 0) {
//        usleep(microSecondsAhead);
//    }
//    tv_prev = tv_now;
}
...
int graphics_3d_subinit(uint32_t *Rmask, uint32_t *Gmask, uint32_t *Bmask,
    ...
    ppb_g3d_interface = (PPB_Graphics3D *) NaCl_GetInterface(PPB_GRAPHICS_3D_INTERFACE);
    if (!ppb_g3d_interface) {
        DEBUG_LOG("Could not acquire PPB_Graphics3D interface.\n");
        return 0;
    }
    ...
    graphics_context = ppb_g3d_interface->Create(
            ...
            attribs);
    if (!graphics_context) {
        DEBUG_LOG("Could not obtain a PPB_Graphics3D context.\n");
        return 0;
    }
    ...
    gfxvidinfo.bufmem = (uae_u8 *) calloc(round_up_to_power_of_2(gfxvidinfo.rowbytes),
    ...
    *Rmask = 0x0000F800, *Gmask = 0x000007E0, *Bmask = 0x0000001F, *Amask = 0;
    ...
    /* The following portion was written with help from these pages:
     * http://stackoverflow.com/questions/4036737/how-to-draw-a-texture-as-a-2d-background-in-opengl-es-2-0
     * http://stackoverflow.com/questions/9355780/how-to-crop-and-scale-a-texture-in-opengl.
    ...
    glBindAttribLocation(g_programObj, ATTRIB_TEXTUREPOSITON, "inputTextureCoordinate");

    glLinkProgram(g_programObj);

    uniforms[UNIFORM_VIDEOFRAME] = glGetUniformLocation(g_programObj, "videoFrame");
    uniforms[UNIFORM_INPUTCOLOR] = glGetUniformLocation(g_programObj, "inputColor");
    uniforms[UNIFORM_THRESHOLD] = glGetUniformLocation(g_programObj, "threshold");
